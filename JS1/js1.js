...
function jose(){
    let n0a = parseInt(n0.value);
    salidajose.innerHTML = n0a;
}


function ejercicio1() {
    let numer1 = parseInt(document.getElementById('entrada1a').value);
    let numer2 = parseInt(document.getElementById('entrada1b').value);
    let numer3 = parseInt(document.getElementById('entrada1c').value);

    let total1 = ((numer1 + numer2 + numer3)).toFixed(2);
    alert(total1);
...
function ejercicio2() {
    let arrayDeNumeros = [];
    const numerosMultiplicados = [1,2,3,4,5].map((numero)=>{
        let valor = parseInt(document.getElementById(`n${numero}`).value);
        arrayDeNumeros.push(valor)
        return numero*2    
            })
    console.log([1,2,3,4,5].map(numero => numero*2))
    
    const reducer = (accumulator, curr) => accumulator + curr;
    const suma = arrayDeNumeros.reduce(reducer);

    let resultado = suma / arrayDeNumeros.length;
